...
example : min a b = min b a := by
  have h : ∀ x y : ℝ, min x y ≤ min y x := by
    ...
    apply min_le_left
  apply le_antisymm
  apply h -- here it is apply h to a and b
  apply h -- here it is apply h to b and a

example : min a b = min b a := by
  apply le_antisymm
  repeat -- apply a tactic as many times as it can
    apply le_min
    apply min_le_right
    apply min_le_left
...
example : min (min a b) c = min a (min b c) := by -- so hard!!!
  apply le_antisymm
...
example : Nat.gcd m n = Nat.gcd n m := by
  apply dvd_antisymm -- use the anti-symmetry of divisibility
  repeat'
    apply dvd_gcd
    apply gcd_dvd_right
    apply gcd_dvd_left
