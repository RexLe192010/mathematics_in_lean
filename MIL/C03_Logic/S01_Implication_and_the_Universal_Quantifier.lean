...
theorem my_lemma2 : ∀ {x y ε : ℝ}, 0 < ε → ε ≤ 1 → |x| < ε → |y| < ε → |x * y| < ε := by
  -- notice that x y ε are surrounded by curly brackets
  apply my_lemma -- call my_lemma to finsh the proof
...
theorem my_lemma3 :
  ...
  intro x y ε epos ele1 xlt ylt
  sorry -- proven in my_lemma

theorem my_lemma4 : -- proven in my_lemma
    ∀ {x y ε : ℝ}, 0 < ε → ε ≤ 1 → |x| < ε → |y| < ε → |x * y| < ε := by
  intro x y ε epos ele1 xlt ylt
  calc
    |x * y| = |x| * |y| := sorry
...
example {c : ℝ} (mf : Monotone f) (nnc : 0 ≤ c) : Monotone fun x ↦ c * f x := by
  intro a b aleb
  apply mul_le_mul_of_nonneg_left -- use the assumption that c is nonnegative
  apply mf
  apply aleb
  apply nnc
